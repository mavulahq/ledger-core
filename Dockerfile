...
RUN npm i -g pnpm@10.33.0 && pnpm install --filter @getfluxo/fengine... --frozen-lockfile --prod=false --ignore-scripts
...
COPY package.json pnpm-lock.yaml pnpm-workspace.yaml .npmrc ./
COPY packages/fengine/package.json packages/fengine/package.json
RUN npm i -g pnpm@10.33.0 && pnpm install --filter @getfluxo/fengine --prod --frozen-lockfile --ignore-scripts
